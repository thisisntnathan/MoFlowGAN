# This version of sparse_molecular_dataset has been adapted from the original code to read validation indices from the provided .json file
...
class SparseMolecularDataset():
    ...
    def load(self, filename, subset=1):
        ...
        # self.__len = self.train_count + self.validation_count 
    ...
    def generate(self, filename, add_h=False, filters=lambda x: True, size=None, validation=0.1, test=0.1):
        ...
        print('Made smiles array of shape: {}'.format(self.smiles.shape))
    ...
    def _generate_train_validation_test(self, validation, test):
        ...
        self.all_idx = np.arange(0, len(self), 1)
        self.validation_idx = get_val_ids()
        self.train_idx = np.delete(self.all_idx, self.validation_idx).tolist()
        self.test_idx = np.random.choice(self.validation_idx, size=(5,), replace=False).tolist()
        self.validation_idx = self.validation_idx.tolist()
        
        test = len(self.test_idx)
        validation = len(self.validation_idx) - test
...
if __name__ == '__main__':
    ...
    # data.generate('qm9.smi', filters=lambda x: x.GetNumAtoms() <= 9)
    # data.save('qm9.sparsedataset')

    # QM9 dataset
    data = SparseMolecularDataset()
    # data.generate('qm9.smi', validation=0.000021, test=0.000021, filters=lambda x: x.GetNumAtoms() <= 9)
    # data.generate('qm9.smi', filters=lambda x: x.GetNumAtoms() <= 9)
    data.generate('qm9.smi', validation=0.000021, test=0.000021)
    data.save('qm9.sparsedataset')
